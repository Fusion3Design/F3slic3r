...
std::pair<std::string, bool> SlicingProcessCompletedEvent::format_error_message() const
{
	...
	try {
        ...
        error = GUI::format(_L("F3Slic3r has encountered a fatal error: \"%1%\""), ex.what()) + "\n\n" +
        		_u8L("Please save your project and restart F3Slic3r. "
...
void BackgroundSlicingProcess::finalize_gcode()
{
	...
	std::string output_path_orig = m_temp_output_path;
